...
# Importação da nova função de teclado inline para "Ver Produtos"
...
def generar_cobranca(call: types.CallbackQuery, produto_id: int):
    ...
    try:
        ...
        with conn:
            ...
            if is_sqlite and not isinstance(cur, sqlite3.Row): 
...
def index():
    ...
    try:
        ...
        with conn:
            ...
            # Ensure end_of_current_month is correctly calculated to the last day of the month
            ...
            start_of_previous_month = (start_of_current_month - timedelta(days=1)).replace(day=1) # Go back to start of prev month
            end_of_previous_month = (start_of_current_month - timedelta(microseconds=1)).replace(hour=23, minute=59, second=59, microsecond=999999) # End of prev month
...
def send_welcome(message):
    ...
    # AQUI ESTÁ A MUDANÇA: REATIVAR O TECLADO PRINCIPAL
    # E PASSAR O TECLADO INLINE PARA O BOTÃO "Ver Produtos"
    # ATENÇÃO: Se `menu_principal()` retornar None (como no último `keyboards.py`),
    # e você quiser apenas o botão inline, remova `menu_principal()` daqui.
    # Se quiser o teclado Reply (menu) E o inline button na mensagem,
    # então teremos que enviar DUAS MENSAGENS.

    # Opção 1: Mensagem de boas-vindas SEM teclado Reply, e apenas o botão inline de produtos.
    bot.reply_to(message, formatted_message) # Sem reply_markup aqui

    # Opção 2: Se você quer uma mensagem separada com o botão inline "Ver Produtos"
    # Você terá que enviar uma segunda mensagem ou modificar mostrar_produtos_bot_func para
    # anexar o inline_ver_produtos_keyboard() a um cabeçalho de produtos.

    # Se a intenção é que o /start liste os produtos diretamente com seus botões DE COMPRA (inline)
    # e não tenha o teclado REPLY ("Ver Produtos"), a lógica atual está quase lá.
    # A imagem mostra que o botão "Ver Produtos" é um REPLY KEYBOARD, não inline.
    # Então, se o desejo é tê-lo INLINE, precisamos mudar o lugar onde ele é adicionado.

    # CORREÇÃO PARA O BOTÃO "VER PRODUTOS" SER INLINE NA MENSAGEM DE BOAS-VINDAS
    # Se você quer o botão inline "Ver Produtos" na mensagem de boas-vindas:
    bot.reply_to(message, formatted_message, reply_markup=inline_ver_produtos_keyboard())

    # NÃO CHAME mostrar_produtos_bot_func AQUI SE VOCÊ QUISER O BOTÃO "VER PRODUTOS"
    # PARA LISTAR PRODUTOS DEPOIS. Se quiser listar produtos automaticamente NO /start,
    # então não precisa do botão "Ver Produtos" (ou ele seria redundante).
    # O user pediu para listar no /start E ter o botão. Isso pode ser confuso.
    # Vamos interpretar que o user quer o botão "VER PRODUTOS" inline, e ao clicar, lista.
    # REMOVIDO: mostrar_produtos_bot_func(message.chat.id) daqui se o botão for para LISTAR PRODUTOS.
...
if __name__ != '__main__':
    ...
    try:
        ...
        # CHAMA register_produtos_handlers E ARMAZENA A FUNÇÃO RETORNADA
        # Esta variável é usada apenas se o `send_welcome` chamar diretamente a listagem de produtos.
        # Se a listagem agora é via botão inline, esta atribuição ainda é válida
        # mas `send_welcome` não a chamará diretamente.
        global mostrar_produtos_bot_func 
        mostrar_produtos_bot_func = register_produtos_handlers(bot, get_db_connection, generar_cobranca) 
