...
def init_db():
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            # --- FIM DA NOVA TABELA E ALTERAÇÃO ---
...
# Mover esta função para cima para que seja definida antes da inicialização
...
#                         try:
#                             if msg.image_url:
#                                 print(f"DEBUG WORKER: Enviando foto para {chat_id} com URL: {msg.image_url}")
...
Obrigado por me enviar o seu `app.py` mais recente.

A imagem que você compartilhou indica que há **problemas de indentação e caracteres inválidos** na seção `if __name__ != '__main__':` do seu `app.py`. Isso impede que seu aplicativo Python seja executado corretamente e, consequentemente, que o worker de mensagens agendadas e qualquer nova funcionalidade funcionem.

Eu vou corrigir esses problemas no seu `app.py` no Canvas, **reintroduzindo o worker de mensagens agendadas e adicionando a funcionalidade de Broadcast (envio em massa) com suporte a imagem/vídeo**.

### Detalhes das Modificações no Canvas `app.py`:

1.  **Correção de Indentação:** O bloco `if __name__ != '__main__':` será reindentado corretamente.
2.  **Remoção de Caracteres Inválidos:** Quaisquer caracteres invisíveis ou problemáticos serão removidos.
3.  **Reinclusão do `scheduled_message_worker`:** A função `scheduled_message_worker` e a lógica para iniciá-lo como uma `Thread` em segundo plano (`worker_thread = Thread(...)`) serão adicionadas de volta à seção `if __name__ != '__main__':`. Isso fará com que as mensagens agendadas sejam enviadas automaticamente.
4.  **Adição da Coluna `image_url` no `init_db()`:** Para a tabela `scheduled_messages`, será adicionado o código para garantir que a coluna `image_url` exista (usando `ALTER TABLE ADD COLUMN IF NOT EXISTS`) e assim permitir o armazenamento de URLs de imagem/vídeo.
5.  **Nova Rota `/send_broadcast`:**
    * Uma nova rota no Flask para o painel de administração (`/send_broadcast`) será criada.
    * Ela terá métodos `GET` (para exibir o formulário) e `POST` (para processar o envio).
    * A lógica de `POST` irá:
        * Obter o `message_text` e a `image_url` do formulário.
        * Iterar sobre **todos os usuários registrados na sua tabela `users`**.
        * Enviar a mensagem (com a imagem/vídeo, se fornecida) para cada usuário individualmente usando `bot.send_message` ou `bot.send_photo`.
        * Lidar com possíveis erros de envio (ex: usuário bloqueou o bot).
6.  **Novo Item no Menu do `layout.html`:** Adicionarei um novo link para `/send_broadcast` no menu lateral do `layout.html` para que você possa acessar facilmente o formulário de broadcast.
7.  **Novo Template `templates/send_broadcast.html`:** Este arquivo será o formulário para a funcionalidade de broadcast.

---

### **Código `app.py` ATUALIZADO (Completo com as Correções)**
...
# --- WORKER PARA ENVIO DE MENSAGENS AGENDADAS ---
# (Esta função será movida para o topo do arquivo para melhor organização)
def scheduled_message_worker():
    print("DEBUG WORKER: Iniciando worker de mensagens agendadas...")
    while True:
        conn = None
        try:
            ...
            with conn.cursor() as cur:
                ...
                # E que não falharam muitas vezes (opcional, para evitar loops infinitos em erros persistentes)
                cur.execute(
                    "SELECT id, message_text, target_chat_id, image_url FROM scheduled_messages WHERE status = 'pending' AND schedule_time <= NOW() ORDER BY schedule_time ASC"
                )
                messages_to_send = cur.fetchall()
                    ...
                    for chat_id in target_chats:
                        try:
                            ...
                            if msg.image_url:
                                ...
                                # O send_photo pode lidar com URLs de vídeo se o Telegram as aceitar como foto para preview
                                bot.send_photo(chat_id, msg.image_url, caption=msg.message_text, parse_mode='Markdown')
                            else:
                                print(f"DEBUG WORKER: Enviando texto para {chat_id}")
                                bot.send_message(chat_id, msg.message_text, parse_mode='Markdown')
            ...
            with conn.cursor() as cur:
                ...
                for user_id in target_user_ids:
                    try:
                        ...
                        traceback.print_exc() # Imprime o stack trace completo
                        if "chat not found" in str(e).lower() or "bot was blocked by the user" in str(e).lower() or "user is deactivated" in str(e).lower():
                            print(f"AVISO BROADCAST: Usuário {user_id} pode ter bloqueado o bot ou não existe.")
                        else:
                            failed_sends += 1
                    except Exception as e:
