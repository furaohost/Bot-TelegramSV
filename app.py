import os
import sqlite3 # Manter por enquanto, pode ser removido depois se não usar SQLite em mais nada
import json
import requests
import telebot
...
# DB_NAME = 'dashboard.db' # Manter para a lógica do caminho, mas não será o nome do arquivo SQLite
...
def init_db():
    ...
    try:
        ...
        cur = conn.cursor() # <--- AQUI: CRIAR CURSOR PARA init_db
...
def webhook_mercado_pago():
    ...
    if notification and notification.get('type') == 'payment':
        ...
        if payment_info and payment_info['status'] == 'approved':
            ...
            cur = conn.cursor() # <--- AQUI: CRIAR CURSOR
            venda = cur.execute('SELECT * FROM vendas WHERE id = %s AND status = %s', (venda_id, 'pendente')).fetchone() # <--- USAR CURSOR E %s PARA PLACEHOLDER
            ...
            if venda:
                ...
                if datetime.now() > data_venda_dt + timedelta(hours=1):
                    ...
                    cur.execute('UPDATE vendas SET status = %s WHERE id = %s', ('expirado', venda_id)) # <--- USAR CURSOR E %s
                    conn.commit()
                    cur.close()
                    conn.close()
                ...
                cur.execute('UPDATE vendas SET status = %s, payment_id = %s, payer_name = %s, payer_email = %s WHERE id = %s', 
                                 ('aprovado', payment_id, payer_name, payer_email, venda_id)) # <--- USAR CURSOR E %s
                conn.commit()
                produto = cur.execute('SELECT * FROM produtos WHERE id = %s', (venda['produto_id'],)).fetchone() # <--- USAR CURSOR E %s
                cur.close()
                conn.close()
                if produto:
...
def login():
    ...
    if request.method == 'POST':
        ...
        conn = get_db_connection()
        cur = conn.cursor() # <--- AQUI: CRIAR CURSOR
        admin_user = cur.execute('SELECT * FROM admin WHERE username = %s', (username,)).fetchone() # <--- USAR CURSOR E %s
        cur.close() # Fechar cursor antes de fechar a conexão
        conn.close()
...
def index():
    ...
    for i in range(6, -1, -1):
        ...
        daily_revenue = cur.execute("SELECT SUM(preco) FROM vendas WHERE status = %s AND data_venda BETWEEN %s AND %s", ('aprovado', start_of_day, end_of_day)).fetchone()[0] # <--- USAR CURSOR E %s
...
def produtos():
    ...
    if request.method == 'POST':
        ...
        conn = get_db_connection()
        cur = conn.cursor() # <--- AQUI: CRIAR CURSOR
        cur.execute('INSERT INTO produtos (nome, preco, link) VALUES (%s, %s, %s)', (nome, preco, link)) # <--- USAR CURSOR E %s
        conn.commit()
        cur.close() # Fechar o cursor
        conn.close()
    ...
    conn = get_db_connection()
    cur = conn.cursor() # <--- AQUI: CRIAR CURSOR
    lista_produtos = cur.execute('SELECT * FROM produtos ORDER BY id DESC').fetchall() # <--- USAR CURSOR
    cur.close() # Fechar o cursor
    conn.close()
...
def edit_product(id):
    ...
    conn = get_db_connection()
    cur = conn.cursor() # <--- AQUI: CRIAR CURSOR
    produto = cur.execute('SELECT * FROM produtos WHERE id = %s', (id,)).fetchone() # <--- USAR CURSOR E %s
    if request.method == 'POST':
        nome, preco, link = request.form['nome'], request.form['preco'], request.form['link']
        cur.execute('UPDATE produtos SET nome = %s, preco = %s, link = %s WHERE id = %s', (nome, preco, link, id)) # <--- USAR CURSOR E %s
        conn.commit()
        cur.close() # Fechar o cursor
        conn.close()
...
def remove_product(id):
    ...
    conn = get_db_connection()
    cur = conn.cursor() # <--- AQUI: CRIAR CURSOR
    cur.execute('DELETE FROM produtos WHERE id = %s', (id,)) # <--- USAR CURSOR E %s
    conn.commit()
    cur.close() # Fechar o cursor
    conn.close()
...
def vendas():
    ...
    if data_inicio_str: conditions.append("DATE(T.data_venda) >= %s"); params.append(data_inicio_str) # <--- USAR %s
    if data_fim_str: conditions.append("DATE(T.data_venda) <= %s"); params.append(data_fim_str) # <--- USAR %s
    if pesquisa_str: conditions.append("(T.username ILIKE %s OR T.nome ILIKE %s OR T.first_name ILIKE %s)"); params.extend([f'%{pesquisa_str}%'] * 3) # <--- USAR ILIKE para case-insensitive no PostgreSQL
    if produto_id_str: conditions.append("T.produto_id = %s"); params.append(produto_id_str) # <--- USAR %s
    if status_str: conditions.append("T.status = %s"); params.append(status_str) # <--- USAR %s
    
    if conditions: query_base += " WHERE " + " AND ".join(conditions)
    query_base += " ORDER BY T.id DESC"
    
    lista_vendas = cur.execute(query_base, tuple(params)).fetchall() # <--- USAR CURSOR E %s
    cur.close() # Fechar o cursor
    conn.close()
...
def venda_detalhes(id):
    ...
    conn = get_db_connection()
    cur = conn.cursor() # <--- AQUI: CRIAR CURSOR
    venda = cur.execute('SELECT * FROM vendas WHERE id = %s', (id,)).fetchone() # <--- USAR CURSOR E %s
    cur.close() # Fechar o cursor
    conn.close()
...
def usuarios():
    ...
    conn = get_db_connection()
    cur = conn.cursor() # <--- AQUI: CRIAR CURSOR
    lista_usuarios = cur.execute('SELECT * FROM users ORDER BY id DESC').fetchall() # <--- USAR CURSOR
    cur.close() # Fechar o cursor
    conn.close()
...
def remove_user(id):
    ...
    conn = get_db_connection()
    cur = conn.cursor() # <--- AQUI: CRIAR CURSOR
    cur.execute('DELETE FROM users WHERE id = %s', (id,)) # <--- USAR CURSOR E %s
    conn.commit()
    cur.close() # Fechar o cursor
    conn.close()
...
def config_messages():
    ...
    current_welcome_message_bot = cur.execute("SELECT value FROM config WHERE key = %s", ('welcome_message_bot',)).fetchone() # <--- USAR CURSOR E %s
    current_welcome_message_bot = current_welcome_message_bot['value'] if current_welcome_message_bot else '' # <--- ACESSAR POR CHAVE NO REALDICTCURSOR

    if request.method == 'POST':
        new_message = request.form['welcome_message_bot']
        # Atualizar a mensagem no banco de dados
        cur.execute("UPDATE config SET value = %s WHERE key = %s", (new_message, 'welcome_message_bot')) # <--- USAR CURSOR E %s
        conn.commit()
        cur.close() # Fechar o cursor
        conn.close()
...
def send_welcome(message):
    ...
    # Lógica para obter a mensagem de boas-vindas do DB (agora do PostgreSQL)
    conn = get_db_connection()
    cur = conn.cursor() # <--- AQUI: CRIAR CURSOR
    welcome_message = cur.execute("SELECT value FROM config WHERE key = %s", ('welcome_message_bot',)).fetchone() # <--- USAR CURSOR E %s
    cur.close() # Fechar o cursor
    conn.close()

    final_message = "Olá! Bem-vindo(a)." # Mensagem padrão de fallback
    if welcome_message:
        final_message = welcome_message['value'].replace('{first_name}', message.from_user.first_name or 'usuário') # <--- ACESSAR POR CHAVE NO REALDICTCURSOR
    else:
...
def mostrar_produtos(chat_id):
    conn = get_db_connection()
    cur = conn.cursor() # <--- AQUI: CRIAR CURSOR
    produtos = cur.execute('SELECT * FROM produtos').fetchall() # <--- USAR CURSOR
    cur.close() # Fechar o cursor
    conn.close()
    if not produtos:
...
def gerar_cobranca(call: types.CallbackQuery, produto_id: int):
    ...
    conn = get_db_connection()
    cur = conn.cursor() # <--- AQUI: CRIAR CURSOR
    produto = cur.execute('SELECT * FROM produtos WHERE id = %s', (produto_id,)).fetchone() # <--- USAR CURSOR E %s
    ...
    cur.execute("INSERT INTO vendas (user_id, produto_id, preco, status, data_venda) VALUES (%s, %s, %s, %s, %s) RETURNING id",
                     (user_id, produto['id'], produto['preco'], 'pendente', data_venda)) # <--- USAR CURSOR, %s e RETURNING id
    ...
    # conn.close() # Já está fechado acima
